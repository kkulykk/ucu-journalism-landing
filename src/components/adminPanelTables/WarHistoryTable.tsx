...
const WarHistoryTable = () => {
  const [warHistoryAdminPanelObject, setWarHistoryAdminPanelObject] = useState<
    WarHistoryObj[]
  >([]);
    ...
    <TableContainer sx={{ height: 250, background: "aqua" }}>
      <Table>
        <TableHead>
          <TableRow>
            {columns.map((column) => (
              <TableCell key={column.id} sx={{ top: 57 }}>
                {column.label}
              </TableCell>
            ))}
          </TableRow>
      ...
      <WarHistoryLeaderInterviewEditModal
        modalHeading={"War History Post | Id: " + "ID"}
        title={editModalTitle}
        videoUrl={editModalVideoUrl}
        date={editModalDate}
